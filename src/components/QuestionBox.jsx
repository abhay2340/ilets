...
const QuestionBox = ({ question, answer, setAnswer, onVisited, setAnswerForId, allAnswers }) => {
  ...
  return (
    <div id={`q-${question.id}`} style={{ marginBottom: '25px' }}>
      ...
      {question.type === 'headingmatch' && (() => {
        const bank = Array.isArray(question.options) ? question.options.map((t) => String(t ?? '').trim()).filter(Boolean) : [];
        ...
        return (
          <div style={{ marginTop: 10 }}>
            ...
            <div style={{ fontWeight: 700, marginBottom: 8, fontSize: '14px', color: '#333' }}>List of Headings</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {bankRemaining.length === 0 && (
                <span style={{ color: '#999', fontStyle: 'italic', fontSize: '13px' }}>All headings placed</span>
              )}
            </div>
          </div>
        );

...
const QuestionBox = ({ question, answer, setAnswer, onVisited, setAnswerForId, allAnswers }) => {
  ...
  if (question.type === 'info') {
    ...
    return (
      <div style={{ margin: '12px 0 6px 0' }}>
        <div
          className="rich-question-content"
          style={{ fontWeight: isBold ? 800 : 400, fontSize: '16px' }}
  ...
  const hasInlineBlanks = /_{3,}/.test(stripHtml(question.question));
  const hideHeader =
    ...
    <div id={`q-${question.id}`} style={{ marginBottom: '25px' }}>
      {!hideHeader && (
        <div style={{ marginBottom: '10px' }}>
          <strong>{question.displayId ?? question.id}. </strong>
          <span
            className="rich-question-content"
            dangerouslySetInnerHTML={{ __html: question.question || '' }}
          />
      ...
      {question.type === 'written' &&
        (() => {
          ...
          if (blankRegex.test(stripHtml(question.question))) {
            ...
            const [localVals, setLocalVals] = React.useState(
              vals.length ? vals : Array(chunks.length - 1).fill('')
            );
            React.useEffect(() => {
              const next =
                typeof answer === 'string' && answer.length
                  ? answer.split('|')
                  : Array(chunks.length - 1).fill('');
              setLocalVals(next);
              ...
              <div style={{ marginTop: '10px', marginLeft: '10px', lineHeight: 1.8 }}>
                ...
                {chunks.map((c, i) => {
                  if (i === chunks.length - 1)
                    return <span key={`w-c-${i}`} dangerouslySetInnerHTML={{ __html: c }} />;
                  return (
      ...
      {question.type === 'matchinggroup' &&
        (() => {
          ...
          const parsedCols = React.useMemo(
            () =>
              cols.map((c, idx) => {
          ...
          const choose = (rowIdx, colLetter) => {
            ...
            try {
              if (
                Array.isArray(question.subIds) &&
                typeof question.subIds[rowIdx] === 'number' &&
                typeof setAnswerForId === 'function'
              ) {
            ...
            <div style={{ marginTop: '10px', overflowX: 'auto' }}>
              ...
              <table style={{ borderCollapse: 'collapse', minWidth: '420px' }}>
                <thead>
                  <tr>
                    <th
                      style={{ textAlign: 'left', padding: '8px', borderBottom: '2px solid #000' }}
                    ></th>
                    {parsedCols.map((c, i) => (
                      <th
                        key={i}
              ...
              <div style={{ marginTop: '8px', color: '#666', fontSize: '12px' }}>
                Your selections:{' '}
                {localSel.filter(Boolean).length > 0 ? localSel.join(' | ') : 'None'}
              </div>
            </div>
          );
        })()}
          ...
          return (
            <div style={{ marginTop: '10px' }}>
              ...
              {question.question ? (
                <div
                  className="rich-question-content"
                  style={{ marginBottom: 10, fontWeight: 600, color: '#111' }}
                  dangerouslySetInnerHTML={{ __html: question.question }}
                />
              ) : null}
        ...
        (() => {
          ...
          const onDropToRow = (rowIdx, text) => {
            ...
            try {
              ...
              if (
                Array.isArray(question.subIds) &&
                typeof question.subIds.find === 'function' &&
                prevAtRow
              ) {
      ...
      {question.type === 'summarydrag' &&
        (() => {
          const bank = Array.isArray(question.options)
            ? question.options.map((t) => String(t ?? ''))
            : [];
          ...
          const onDropToBlank = (idx, word) => {
            ...
            try {
              if (
                Array.isArray(question.subIds) &&
                typeof question.subIds[idx] === 'number' &&
                typeof setAnswerForId === 'function'
              ) {
          ...
          return (
            <div style={{ marginTop: 10 }}>
              ...
              <div style={{ marginLeft: 10, lineHeight: 1.8 }}>
                {parts.map((chunk, i) => {
                  if (i === parts.length - 1)
                    return <span key={`c-${i}`} dangerouslySetInnerHTML={{ __html: chunk }} />;
                        ...
                        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                          ...
                          >
                            {rowMeta.splits.map((chunk, idx) => {
                              ...
                              const globalIdx =
                                partsPerRow.slice(0, rowIdx).reduce((acc, r) => acc + r.blanks, 0) +
                                runningBlank;
        ...
        (() => {
          ...
          return (
            <div style={{ marginTop: '10px', marginLeft: '10px', lineHeight: 1.8 }}>
              {parts.map((chunk, i) => {
                if (i === parts.length - 1)
                  return <span key={`sf-c-${i}`} dangerouslySetInnerHTML={{ __html: chunk }} />;
                return (
                      ...
                      <tr key={rIdx}>
                        {(rr || []).map((cell, cIdx) => {
                          ...
                          return (
                            <td
                              key={cIdx}
                              style={{ border: '1px solid #e3e3e3', padding: 8, lineHeight: 1.8 }}
                            >
                              {chunks.map((ch, i) => {
                                if (i === chunks.length - 1)
                                  return <span key={`tf-${rIdx}-${cIdx}-${i}`}>{ch}</span>;
                                globalIdx += 1;
                                const idx = globalIdx;
                                return (
      ...
      {question.type === 'dropdown' &&
        (() => {
          ...
          const labelToValue = parsedOptions.reduce((acc, o) => {
            acc[o.label] = o.value;
            return acc;
          }, {});

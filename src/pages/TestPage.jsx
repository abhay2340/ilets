...
import TestPartTabs from '../components/TestPartTabs';
...
import { db, auth } from '../firebaseConfig.jsx'; // make sure this path matches your config
import test4 from '../data/test4.jsx';
import test5 from '../data/test5.jsx';

const TEST_MAP = { test1, test2, test3, test4, test5};

const TestPage = () => {
  const location = useLocation();
  const params = new URLSearchParams(location.search);
  const currentTestId = params.get('testId') || 'test1';
  const testData = TEST_MAP[currentTestId];
// pick the first available audio from any part
const persistentAudioSrc = React.useMemo(() => {
  const withAudio = testData.parts.find(p => p.audioSrc);
...
// --- Listening lock: once started, user can't pause/seek ---
  ...
  const currentPart = testData.parts[partIndex];
...
const timeTaken = TOTAL_DURATION - timeLeftRef.current;
    ...
    const allQuestions = testData.parts.flatMap(p => p.questions);
    const userAnswers = {};

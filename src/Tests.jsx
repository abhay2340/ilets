...
const Tests = () => {
  ...
  return (
    <div style={{ padding: '16px', maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
      ...
      {!loading && !error && tests.length === 0 && <div>No tests found.</div>}
      <div
        style={{
          display: 'grid',
          ...
          <div key={test.id} style={{ border: '1px solid #e0e0e0', borderRadius: 8, padding: 12 }}>
            ...
            <div
              style={{
                ...
                borderRadius: 999,
                background: test.type === 'audio' ? '#e8f5ff' : '#e9ffe9',
                border: `1px solid ${test.type === 'audio' ? '#cfe8ff' : '#c8f2c8'}`,
              }}
            >
              {test.type === 'audio' ? 'Listening' : 'Reading'}
            </div>
            <div style={{ marginTop: 10 }}>
              <button
                onClick={() => navigate(`/admin/${test.id}`)}
                style={{
                  padding: '6px 10px',
                  borderRadius: 6,
              ...
              <button
                onClick={() =>
                  setConfirmDelete({ id: test.id, title: test.title || `#${test.number || ''}` })
                }
                style={{
                  padding: '6px 10px',
                  borderRadius: 6,
        ...
        >
          ...
          >
            ...
            <p style={{ marginTop: 0, marginBottom: 18 }}>
              This will permanently delete “{confirmDelete?.title}”. The associated answer key will

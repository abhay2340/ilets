    ...
    <div style={{ padding: '16px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      {bundle && (
        <div style={{ marginBottom: 24, paddingBottom: 16, borderBottom: '1px solid #eee' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h2 style={{ margin: 0 }}>{bundle.name || 'Package'}</h2>
              <div
                style={{ marginTop: 6, fontSize: '1.2em', fontWeight: 'bold', color: '#b30000' }}
              >
                {Number(bundle.price || 0) > 0
      ...
      {!loading && !error && (
        <div>
          {/* Listening Section */}
          {(() => {
            const listening = tests.filter((t) => t.type === 'audio');
                  ...
                  {listening.map((t) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: 16,
                        background: '#fff9f9',
                        boxShadow: '0 2px 8px rgba(179,0,0,0.05)',
                        opacity: hasBundleAccess ? 1 : 0.6,
                        position: 'relative',
                      }}
                    >
                      <div
          ...
          {/* Reading Section */}
          {(() => {
            const reading = tests.filter((t) => t.type !== 'audio');
                  ...
                  {reading.map((t) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: 16,
                        background: '#f5f9ff',
                        boxShadow: '0 2px 8px rgba(0,96,179,0.05)',
                        opacity: hasBundleAccess ? 1 : 0.6,
                        position: 'relative',
                      }}
                    >
                      ...
                      <div style={{ marginTop: 12 }}>
                        {!user || hasBundleAccess ? (
                          <button
                            ...
                            style={{
                              background: 'linear-gradient(135deg, #0060b3, #004080)',
                              color: '#fff',
                              border: 'none',
                              padding: '8px 12px',
                              borderRadius: 12,
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          ...
          navigate('/login', {
            state: { from: { pathname: '/payment', search: `?bundleId=${bundle?.id}` } },
          });
        }}
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          ...
          setPendingTestId(null);
          navigate('/login', {
            state: { from: { pathname: '/security', search: `?dbId=${testId}` } },
          });
        }}
        title="Login Required"
